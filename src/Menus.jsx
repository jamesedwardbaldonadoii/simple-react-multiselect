...
function Menus({
  options = [], selected = [], isLoading, onSelect, onClearSelected, onRemoveSelected,
}) {
  ...
  const selectedItems = selected.map((i, $index) => (
    <div className="d-flex align-items-center form-check mb-1 w-100 pl-2 py-1 pr-1" key={`${i.key}-${$index}`}>
      ...
      <button className="btn btn-danger btn-sm" onClick={() => onRemoveSelected(i)}><i className="fa fa-trash" /></button>
    </div>
  ));
  ...
  return (
    <>
      <div className="flex-grow-1 w-50 h-100 d-flex flex-column">
        ...
        {isLoading ? <div className="d-flex h-100 align-items-center justify-content-center"><i className="d-block fa fa-spinner fa-spin" /></div>
          : (
            <div className="overflow-auto h-100 p-2 w-100 text-left">
              {' '}
              {items}
              {' '}
            </div>
      ...
      <div className="flex-grow-1 w-50 h-100 d-flex flex-column border-left overflow-hidden">
        ...
        { selectedItems.length
          ? (
            <div className="overflow-auto h-100 px-2 w-100 text-left">
              {' '}
              {selectedItems}
              {' '}
            </div>
          ...
          : (
            <div className="d-flex flex-column h-100 align-items-center justify-content-center">
              <i className="fa fa-filter text-xl text-gray" />
